...
private def nh_all' {args : List StmtExprMd} (h : args.attach.all (fun ⟨a, _⟩ => noHolesMd a) = true) : ∀ a ∈ args, noHolesMd a = true :=
...
private axiom inferArgsTyped_id_ax : ∀ (args : List StmtExprMd) (types : List HighTypeMd) (s : InferHoleState),
    (∀ a ∈ args, noHolesMd a = true) → inferArgsTyped args types s = (args, s)

private theorem mapM_id_of (f : StmtExprMd → InferHoleM StmtExprMd) (args : List StmtExprMd) (s : InferHoleState)
    (h : args.attach.all (fun ⟨a, _⟩ => noHolesMd a) = true)
    (hf : ∀ (a : StmtExprMd) (s : InferHoleState), noHolesMd a = true → f a s = (a, s)) :
...
private theorem inferArgs_id_of (args : List StmtExprMd) (s : InferHoleState)
    (h : args.attach.all (fun ⟨a, _⟩ => noHolesMd a) = true)
    (hf : ∀ (a : StmtExprMd) (et : HighTypeMd) (s : InferHoleState), noHolesMd a = true → inferExpr a et s = (a, s))
    (et : HighTypeMd) :
...
private theorem inferStmtList_id_of (stmts : List StmtExprMd) (s : InferHoleState)
    (h : stmts.attach.all (fun ⟨a, _⟩ => noHolesMd a) = true)
    (hf : ∀ (a : StmtExprMd) (s : InferHoleState), noHolesMd a = true → inferStmt a s = (a, s)) :
...
theorem inferExpr_id (expr : StmtExprMd) (et : HighTypeMd) (s : InferHoleState)
    (h : noHolesMd expr = true) : inferExpr expr et s = (expr, s) := by
  conv => lhs; unfold inferExpr
  match expr, h with
  | ⟨.Hole true _, _⟩, h => simp [noHolesMd_mk, noHoles_hole_true] at h
  | ⟨.Hole false _, _⟩, _ => exact sorry -- noHoles allows Hole false but inferExpr modifies it
  | ⟨.PrimitiveOp op args, md⟩, h => simp only [noHolesMd_mk, noHoles_po] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, (inferArgs_id_of args s h inferExpr_id _).1]
  | ⟨.StaticCall callee args, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_sc] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map]
  ...
  | ⟨.InstanceCall target callee args, md⟩, h => simp only [noHolesMd_mk, noHoles_ic, Bool.and_eq_true] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id target _ s h.1, (inferArgs_id_of args s h.2 inferExpr_id _).1]
  | ⟨.ReferenceEquals l r, md⟩, h => simp only [noHolesMd_mk, noHoles_re, Bool.and_eq_true] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id l _ s h.1, inferExpr_id r _ s h.2]
  | ⟨.IfThenElse c t e, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_ite, Bool.and_eq_true] at h; have hc := h.1.1; have ht := h.1.2; have he := h.2
    cases e with
  ...
  | ⟨.Block stmts label, md⟩, h => simp only [noHolesMd_mk, noHoles_block] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferStmtList_id_of stmts s h inferStmt_id]
  | ⟨.Assign _ value, md⟩, h => simp only [noHolesMd_mk, noHoles_assign] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id value _ s h]
  | ⟨.LocalVariable n ty init, md⟩, h => simp only [noHolesMd_mk, noHoles_lv] at h; cases init with | none => rfl | some i => simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id i _ s (by simp_all)]
  | ⟨.Old v, md⟩, h => simp only [noHolesMd_mk, noHoles_old] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id v _ s h]
  | ⟨.Fresh v, md⟩, h => simp only [noHolesMd_mk, noHoles_fresh] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id v _ s h]
  | ⟨.Assigned n, md⟩, h => simp only [noHolesMd_mk, noHoles_assigned] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id n _ s h]
  | ⟨.ProveBy v p, md⟩, h => simp only [noHolesMd_mk, noHoles_proveby, Bool.and_eq_true] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id v _ s h.1, inferExpr_id p _ s h.2]
  | ⟨.ContractOf ty f, md⟩, h => simp only [noHolesMd_mk, noHoles_contractof] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id f _ s h]
  | ⟨.Forall param trigger body, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_forall, Bool.and_eq_true] at h
    cases trigger with
  ...
  | ⟨.Exists param trigger body, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_exists, Bool.and_eq_true] at h
    cases trigger with
    | none => simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id body (bareType .TBool) s h.2]
    | some t => simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id t defaultHoleType s h.1, inferExpr_id body (bareType .TBool) s h.2]
...
theorem inferStmt_id (stmt : StmtExprMd) (s : InferHoleState)
    (h : noHolesMd stmt = true) : inferStmt stmt s = (stmt, s) := by
  conv => lhs; unfold inferStmt
  match stmt, h with
  | ⟨.LocalVariable n ty (some i), md⟩, h => simp only [noHolesMd_mk, noHoles_lv] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id i _ s (by simp_all)]
  | ⟨.Assign targets value, md⟩, h => simp only [noHolesMd_mk, noHoles_assign] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id value _ s h]
  | ⟨.Block stmts label, md⟩, h => simp only [noHolesMd_mk, noHoles_block] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferStmtList_id_of stmts s h inferStmt_id]
  | ⟨.IfThenElse c t e, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_ite, Bool.and_eq_true] at h; have hc := h.1.1; have ht := h.1.2; have he := h.2
    cases e with
  ...
  | ⟨.While c invs dec body, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_while, Bool.and_eq_true] at h; obtain ⟨⟨⟨hc, hinvs⟩, hdec⟩, hbody⟩ := h
  ...
  | ⟨.Assert c, md⟩, h => simp only [noHolesMd_mk, noHoles_assert] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id c _ s h]
  | ⟨.Assume c, md⟩, h => simp only [noHolesMd_mk, noHoles_assume] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id c _ s h]
  | ⟨.StaticCall callee args, md⟩, h =>
    simp only [noHolesMd_mk, noHoles_sc] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map]
    cases calleeParamTypes s.model callee with
    | some types => have := inferArgsTyped_id_ax args types s (nh_all' h); simp [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, this]
    | none => simp [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, (inferArgs_id_of args s h inferExpr_id _).1]
  | ⟨.Return (some v), md⟩, h => simp only [noHolesMd_mk, noHoles_ret] at h; simp only [bind, StateT.bind, get, MonadState.get, StateT.get, getThe, MonadStateOf.get, pure, StateT.pure, Functor.map, StateT.map, inferExpr_id v _ s (by simp_all)]
  | ⟨.Hole true _, _⟩, h => simp [noHolesMd_mk, noHoles_hole_true] at h
  | ⟨.Hole false _, _⟩, _ => exact sorry -- noHoles allows Hole false but inferStmt modifies it
