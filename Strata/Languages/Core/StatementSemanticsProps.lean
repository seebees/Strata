...
theorem EvalBlockEmpty' {P : PureExpr} {Cmd : Type} {EvalCmd : EvalCmdParam P Cmd}
  ...
  EvalBlock P Cmd EvalCmd extendEval δ σ ([]: (List (Stmt P Cmd))) σ' br δ' → σ = σ' := by
  intros H; cases H <;> simp

theorem EvalStatementsEmpty :
  EvalStatements π extendEval δ σ [] σ' br δ' → σ = σ' := by
  intros H; cases H <;> simp

theorem EvalStatementsContractEmpty :
  EvalStatementsContract π extendEval δ σ [] σ' br δ' → σ = σ' := by
  intros H; cases H <;> simp
...
theorem EvalStatementsContractApp' {φ : CoreEval → PureFunc Expression → CoreEval} {δ δ'' : CoreEval} :
  EvalStatementsContract π φ δ σ (ss₁ ++ ss₂) σ'' br δ'' →
  ∃ σ' br₁ δ',
...
theorem EvalStatementsContractApp {φ : CoreEval → PureFunc Expression → CoreEval} {δ δ' δ'' : CoreEval} :
  ...
  EvalStatementsContract π φ δ' σ' ss₂ σ'' br δ'' →
  EvalStatementsContract π φ δ σ (ss₁ ++ ss₂) σ'' br δ'' := by
  intros Heval1 Heval2
  induction ss₁ generalizing σ σ' δ δ' <;> simp_all
  case nil =>
    have ⟨Hσ, Hδ, _⟩ := EvalBlockEmpty Heval1
    simp [Hσ, Hδ]
    assumption
  case cons h t ih =>
    cases Heval1 with
    | stmts_normal_sem Heval Heval' =>
    next σ₁ δ₁ =>
    constructor
    . exact Heval
...
theorem EvalStatementsApp {φ : CoreEval → PureFunc Expression → CoreEval} {δ δ' δ'' : CoreEval} :
  EvalStatements π φ δ σ ss₁ σ' .normal δ' →
  EvalStatements π φ δ' σ' ss₂ σ'' br δ'' →
  EvalStatements π φ δ σ (ss₁ ++ ss₂) σ'' br δ'' := by
  intros Heval1 Heval2
