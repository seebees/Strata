...
**Decision: Predefined Core read functions with fixed axioms.**

**Rationale:** This follows the established layered architecture:
Core provides fixed axioms, Laurel exposes constrained types
paired with those axioms, compilers select. Nobody generates
axioms. The trust surface is a small, enumerable set of Core
functions defined in the Factory — the same place where map and
Sequence axioms live.

The axioms are sound under three conditions (see design.md):
...
For any generated program, correctness can be audited at each
layer independently. If the translator is formally verified, the
correct pairing of constrained types to read functions can be
proven.
