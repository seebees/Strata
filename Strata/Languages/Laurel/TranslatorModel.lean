...
public def translateProgramModel (program : Program) : Core.Program :=
  ...
  let datatypeDecls := datatypes.map fun dt =>
    ...
    let constrs : List (Lambda.LConstr Unit) := if constrs.isEmpty then
      [{ name := ⟨s!"Mk{dt.name.text}", ()⟩, args := [], testerName := "" }]
    else constrs
    Core.Decl.type (.data [{
      name := dt.name.text
      typeArgs := []
  ...
  -- Fixed infrastructure datatypes (generated by heapParameterization)
  let typeTagDecl := Core.Decl.type (.data [{
    name := "TypeTag", typeArgs := [], constrs := [{ name := ⟨"MkTypeTag", ()⟩, args := [], testerName := "" }],
    constrs_ne := by decide }])
  let fieldDecl := Core.Decl.type (.data [{
    name := "Field", typeArgs := [], constrs := [{ name := ⟨"MkField", ()⟩, args := [], testerName := "" }],
    constrs_ne := by decide }])
  let compositeDecl := Core.Decl.type (.data [{
    name := "Composite", typeArgs := [],
    constrs := [{ name := ⟨"MkComposite", ()⟩, args := [(⟨"ref", ()⟩, Lambda.LMonoTy.int)], testerName := "Composite..isMkComposite" }],
    constrs_ne := by decide }])
  let boxDecl := Core.Decl.type (.data [{
    name := "Box", typeArgs := [], constrs := [{ name := ⟨"MkBox", ()⟩, args := [], testerName := "" }],
    constrs_ne := by decide }])
  let heapDecl := Core.Decl.type (.data [{
    name := "Heap"
    typeArgs := []
    constrs := [{
      name := ⟨"MkHeap", ()⟩
      args := [(⟨"data", ()⟩, Lambda.LMonoTy.tcons "Map" [Lambda.LMonoTy.tcons "Composite" [], Lambda.LMonoTy.tcons "Map" [Lambda.LMonoTy.tcons "Field" [], Lambda.LMonoTy.tcons "Box" []]]),
               (⟨"nextReference", ()⟩, Lambda.LMonoTy.int)]
      testerName := "Heap..isMkHeap"
    }]
    constrs_ne := by decide
  }])
  let notSupportedDecl := Core.Decl.type (.data [{
    name := "NotSupportedYet", typeArgs := [],
    constrs := [{ name := ⟨"MkNotSupportedYet", ()⟩, args := [], testerName := "" }],
    constrs_ne := by decide }])
