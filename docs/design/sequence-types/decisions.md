...
## Decision 6: Typed select functions for constrained element types

**Date:** 2026-03-29

### Context

`Sequence.select : ∀a. Sequence a → int → a` returns the element
with no type information. When a language compiler models a typed
array (Java `int[]`, Rust `[i32]`, Python `bytes`) as a Sequence,
the element read produces an unbounded value. The prover can't
prove the element satisfies the constrained type's bounds.

The heap model solved the analogous problem for instance fields
using Factory read functions with fixed axioms (D4 in
constrained-types-in-heap/decisions.md). The question: how should
Sequence element reads carry bounds?

### Options considered

**A. Identity axiom on existing read functions.**

Wrap `Sequence.select` in the existing `readInt32`:
`readInt32(Sequence.select(s, i))` and add an identity axiom
`∀ v: int. readInt32(v) == v`.

Rejected: **unsound.** The identity axiom combined with the bound
axioms asserts that every integer fits in int32 range. For example,
`readInt32(3000000000) == 3000000000` (identity) AND
`readInt32(3000000000) <= 2147483647` (bound) produces
`3000000000 <= 2147483647`, a contradiction. The same problem
applies to `readInt16` and `readInt8`.

The existing Box equality axiom `readInt32(BoxInt(v)) == v` is safe
because it's scoped to `BoxInt` — the prover can only apply it
when the argument is actually a `BoxInt`. The identity axiom has
no such scoping.

**B. Sequence of Boxes.**

Store array elements as `Sequence Box` instead of `Sequence int`.
Then `Sequence.select` returns a `Box`, and the existing
`readInt32(box)` pattern works.

Rejected: couples Core's Sequence model to Laurel's heap/Box
system. Sequences are language-agnostic — a Python backend uses
them for lists without Box. Also adds Box wrapping/unwrapping
overhead to every array operation and requires rewriting the
Sequence axioms to work with Box element types.

**C. Parameterized bounded select (selectBounded with lo/hi).**

A single `Sequence.selectBounded(s, i, lo, hi)` function where
the compiler passes the bounds.

Rejected for the same reason D4 in constrained-types-in-heap
rejected `readBoundedInt(box, lo, hi)`: too flexible. The
compiler could pass any bounds. No way to verify the bounds
match the constrained type without inspecting every call site.

**D. Predefined typed select functions with fixed axioms.**

One Factory function per constrained type:
- `Sequence.selectInt32(s, i)` — result in [-2147483648, 2147483647]
- `Sequence.selectInt16(s, i)` — result in [-32768, 32767]
- `Sequence.selectInt8(s, i)` — result in [-128, 127]

Each has three axioms:
1. Lower bound: `selectInt32(s, i) >= -2147483648`
2. Upper bound: `selectInt32(s, i) <= 2147483647`
3. Equality: `selectInt32(s, i) == Sequence.select(s, i)`

The equality axiom is safe because it's scoped to the specific
function — it says "selectInt32 returns the same value as
Sequence.select," not "all integers are bounded." The bound
axioms only apply to the output of `selectInt32`.

The compiler selects which function to call based on the array
element type: Java `int[]` → `selectInt32`, `short[]` →
`selectInt16`, `byte[]` → `selectInt8`. Python `bytes` →
`selectInt8`. The compiler cannot modify the axioms.

**Decision: D**

**Rationale:** This follows the same pattern as D4 in
constrained-types-in-heap: predefined Factory functions with
fixed axioms, where the compiler selects which to call. The
key properties:

- **Fixed bounds, auditable.** The axioms are in the Factory,
  not generated by the compiler or translator.
- **Scoped equality.** The equality axiom connects the typed
  select to the generic `Sequence.select` without making
  universal claims about all integers.
- **Language-agnostic.** The typed selects are mathematical
  facts about bounded selection from sequences. Any language
  with bounded integer array types can use them. The generic
  `Sequence.select` remains available for unbounded types.
- **No coupling.** Sequences stay independent of Box/heap.
  The typed selects are peers of `Sequence.select` in the
  Factory, not wrappers around the heap model.
- **Consistent architecture.** The same pattern (predefined
  functions, fixed axioms, compiler selects) is used for
  heap field reads and sequence element reads.
