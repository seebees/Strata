...
> **⚠ Revisited in Decision 9.** This choice conflated instance
> method names with datatype destructor names by using the same
> `..` separator for both. These are semantically different
> operations (see Decision 9). The separator for instance methods
> should be changed to `~>` to distinguish them from destructors.
...
## Decision 9: Separate instance method names from destructor names

**Date:** 2026-04-12

### Context

Decision 3 chose `..` as the separator for instance procedure
Core names (`Counter..increment`) because it matched the existing
convention for datatype destructors (`Color..isRed`, `Box..intVal!`).
Decision 8 chose `~>` for Laurel source text syntax because `..`
doesn't work in the tokenizer.

This created a situation where `..` is used for two semantically
different operations:

**Datatype destructors/testers** are pure functions on immutable
value types. `Color..isRed(x)` tests whether `x` was constructed
with the `Red` constructor. `IntList..head(xs)` extracts the first
element. There is no object, no heap, no `self`. The prefix
(`Color..`) is a type name used for namespacing — it prevents
`Color..isRed` from colliding with `Shape..isRed`.

**Instance method calls** are procedure calls on heap-allocated
composite objects. `counter~>increment()` reads or writes the heap
through a receiver reference. There is a `self` parameter. The
heap parameterization pass injects `$heap`. The modifies clause
tracks mutations.

| | Destructor | Instance call |
|---|---|---|
| Target | Value type (datatype) | Reference type (composite) |
| Heap | No heap involvement | Reads/writes heap |
| Self | No self — takes a value | Has self — receiver object |
| Mutability | Pure function | May mutate |
| Prefix meaning | Type name (namespace) | Receiver expression |

Using `..` for both conflates these. When you see `Foo..bar(x)` in
Core output, you must determine from context whether `Foo` is a
datatype (destructor) or a composite (instance method). They go
through completely different pipeline paths — destructors are
generated by the Core DDM transform, instance methods go through
heap parameterization and the Laurel translator.

### Option A: Keep `..` for both (status quo)

Leave `Counter..increment` and `Color..isRed` using the same
separator. Rely on context to distinguish them.

- Pro: No changes. Consistent visual appearance.
- Con: Ambiguous. Two different semantic operations look the same.
  As the codebase grows, the ambiguity becomes a source of bugs —
  a pass that handles `..` names might treat an instance method
  as a destructor or vice versa.

### Option B: Use `~>` for instance methods, keep `..` for destructors

Change `instanceProcCoreName` to use `~>`:
`Counter~>increment` for instance methods,
`Color..isRed` for destructors.

The `~>` separator is already the Laurel source text syntax for
instance calls (Decision 8). Using it in Core names too means one
separator for instance methods everywhere — source text, Core
names, and (if chosen) frontend-qualified callee names.

- Pro: No ambiguity. `~>` always means instance method. `..`
  always means destructor. The separator carries semantic
  information. The Laurel source syntax and Core names use the
  same convention.
- Con: Changes `instanceProcCoreName` and all tests that check
  Core output containing instance method names. The IM1 proof
  needs updating (trivial — it's `rfl` on a different string).
  Need to verify `~>` doesn't conflict with Core's tokenizer.

### Option C: Use a different separator for one or both

Choose new separators that are visually distinct and semantically
clear. For example, `::` for instance methods (C++ convention),
`#` for destructors, etc.

- Pro: Fresh start, no legacy baggage.
- Con: Gratuitous change. `..` for destructors is established in
  the Core grammar and works fine. `~>` for instance calls is
  established in the Laurel grammar and works fine. Changing both
...
1. `Laurel.lean`: `instanceProcCoreName` — `".."` → `"~>"`
2. `HeapParameterization.lean`: Box destructor names — no change
   (these are destructors, they keep `..`)
3. `InstanceMethodProperties.lean`: IM1 proof — update string
4. Test expectations that check Core output for instance method
   names
5. Verify `~>` doesn't conflict with Core's tokenizer/parser

### Relationship to cross-type resolution

See `design/cross-type-resolution/decisions.md`. If Decision 2
there chooses Option A (frontend qualifies callee names), the
frontend would use `~>` as the separator:
`InstanceCall(target, "Position~>compareTo", args)`. This makes
the qualified name visually identical to the Laurel source syntax
and the Core procedure name — one convention everywhere.
