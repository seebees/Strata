...
theorem eval_stmt_assert_store_cst
  ...
  EvalStmt P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (Cmd.assert l e md)) σ' .normal δ' → σ = σ' := by
  intros Heval; cases Heval with | cmd_sem Hcmd => exact eval_assert_store_cst Hcmd

theorem eval_stmt_assert_eval_cst
  [DecidableEq P.Ident]
  [HasVarsImp P (List (Stmt P (Cmd P)))] [HasVarsImp P (Cmd P)] [HasFvar P] [HasVal P] [HasBool P] [HasNot P] :
  EvalStmt P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (Cmd.assert l e md)) σ' .normal δ' → δ = δ' := by
  intros Heval; cases Heval with | cmd_sem Hcmd => rfl

theorem eval_stmts_assert_store_cst
  [DecidableEq P.Ident]
  [HasVarsImp P (List (Stmt P (Cmd P)))] [HasVarsImp P (Cmd P)] [HasFvar P] [HasVal P] [HasBool P] [HasNot P] :
  EvalBlock P (Cmd P) (EvalCmd P) extendEval δ σ [(.cmd (Cmd.assert l e md))] σ' .normal δ' → σ = σ' := by
  intros Heval; cases Heval with
  | stmts_normal_sem H1 H2 =>
    cases H1 with
    | cmd_sem H3 =>
      cases H2 with
...
theorem eval_stmt_assert_eq_of_pure_expr_eq
  ...
  (EvalStmt P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (Cmd.assert l1 e md1)) σ' .normal δ' ↔
  EvalStmt P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (Cmd.assert l2 e md2)) σ' .normal δ') := by
  intro Hwf
  constructor <;>
  (
    intro Heval
...
theorem eval_stmts_assert_elim
  ...
  [HasVarsImp P (List (Stmt P (Cmd P)))] [HasFvar P] [HasVal P] [HasBool P] [HasNot P] :
  WellFormedSemanticEvalBool δ →
  EvalBlock P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (.assert l1 e md1) :: cmds) σ' .normal δ' →
  EvalBlock P (Cmd P) (EvalCmd P) extendEval δ σ cmds σ' .normal δ' := by
  intros Hwf Heval
  cases Heval with
  | stmts_normal_sem Has1 Has2 =>
...
theorem assert_elim
  ...
  [HasVarsImp P (List (Stmt P (Cmd P)))] [HasFvar P] [HasVal P] [HasBool P] [HasNot P] :
  WellFormedSemanticEvalBool δ →
  EvalBlock P (Cmd P) (EvalCmd P) extendEval δ σ (.cmd (.assert l1 e md1) :: [.cmd (.assert l2 e md2)]) σ' .normal δ' →
  EvalBlock P (Cmd P) (EvalCmd P) extendEval δ σ [.cmd (.assert l3 e md3)] σ' .normal δ' := by
  intro Hwf Heval
...
theorem semantic_eval_eq_of_eval_cmd_set_unrelated_var
  ...
  EvalCmd P δ σ (Cmd.set v e' md) σ' →
  δ σ e = δ σ' e := by
...
theorem eval_cmd_set_comm'
  ...
  δ σ v1 = δ σ2 v1 →
  δ σ v2 = δ σ1 v2 →
  EvalCmd P δ σ (Cmd.set x1 v1 md1) σ1 →
  EvalCmd P δ σ1 (Cmd.set x2 v2 md2) σ' →
  EvalCmd P δ σ (Cmd.set x2 v2 md2') σ2 →
  EvalCmd P δ σ2 (Cmd.set x1 v1 md1') σ'' →
  σ' = σ'' := by
...
theorem eval_stmt_set_comm
  ...
  ¬ x1 ∈ HasVarsPure.getVars v2 →
  ¬ x2 ∈ HasVarsPure.getVars v1 →
  EvalStmt P (Cmd P) (EvalCmd P) evalFun δ σ (.cmd (Cmd.set x1 v1 md1)) σ1 .normal δ1 →
  EvalStmt P (Cmd P) (EvalCmd P) evalFun δ σ1 (.cmd (Cmd.set x2 v2 md2)) σ' .normal δ2 →
  EvalStmt P (Cmd P) (EvalCmd P) evalFun δ σ (.cmd (Cmd.set x2 v2 md2')) σ2 .normal δ3 →
  EvalStmt P (Cmd P) (EvalCmd P) evalFun δ σ2 (.cmd (Cmd.set x1 v1 md1')) σ'' .normal δ4 →
  σ' = σ'' := by
  intro Hwf Hneq Hnin1 Hnin2 Hs1 Hs2 Hs3 Hs4
...
theorem eval_stmts_set_comm
  ...
  ¬ x1 ∈ HasVarsPure.getVars v2 →
  ¬ x2 ∈ HasVarsPure.getVars v1 →
  EvalBlock P (Cmd P) (EvalCmd P) evalFun δ σ [(.cmd (Cmd.set x1 v1 md1)), (.cmd (Cmd.set x2 v2 md2))] σ' .normal δ' →
  EvalBlock P (Cmd P) (EvalCmd P) evalFun δ σ [(.cmd (Cmd.set x2 v2 md2')), (.cmd (Cmd.set x1 v1 md1'))] σ'' .normal δ'' →
  σ' = σ'' := by
  intro Hwf Hneq Hnin1 Hnin2 Heval1 Heval2
  -- Decompose first evaluation: [set x1 v1, set x2 v2]
  cases Heval1 with
  | stmts_normal_sem Hs1 Hrest1 =>
    cases Hrest1 with
    | stmts_normal_sem Hs2 Hempty1 =>
      cases Hempty1
      -- Decompose second evaluation: [set x2 v2, set x1 v1]
      cases Heval2 with
      | stmts_normal_sem Hs3 Hrest2 =>
        cases Hrest2 with
        | stmts_normal_sem Hs4 Hempty2 =>
          cases Hempty2
