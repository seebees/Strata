...
public def translateProgramModel (program : Program) : Core.Program :=
  ...
  let typeTagDecl := Core.Decl.type (.data [{
    name := "TypeTag", typeArgs := [], constrs := [{ name := ⟨"MkTypeTag", ()⟩, args := [] }],
    constrs_ne := by decide }])
  let fieldDecl := Core.Decl.type (.data [{
    name := "Field", typeArgs := [], constrs := [{ name := ⟨"MkField", ()⟩, args := [] }],
    constrs_ne := by decide }])
  let compositeDecl := Core.Decl.type (.data [{
    name := "Composite", typeArgs := [],
    constrs := [{ name := ⟨"MkComposite", ()⟩, args := [(⟨"ref", ()⟩, Lambda.LMonoTy.int), (⟨"typeTag", ()⟩, Lambda.LMonoTy.tcons "TypeTag" [])], testerName := "Composite..isMkComposite" }],
    constrs_ne := by decide }])
  ...
  let notSupportedDecl := Core.Decl.type (.data [{
    ...
    constrs := [{ name := ⟨"MkNotSupportedYet", ()⟩, args := [] }],
    constrs_ne := by decide }])
