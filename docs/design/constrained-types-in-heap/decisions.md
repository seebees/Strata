...
For example, `int32` is paired with:
```
function readInt32(box: Box) : int
  axiom: readInt32(box) >= -2147483648
  axiom: readInt32(box) <= 2147483647
  axiom: readInt32(box) == Box..intVal!(box)
```

These functions are defined in Core's Factory alongside the map
and Sequence axioms. The bounds are fixed. The translator selects
which function to call based on the field's constrained type.

**Options considered:**

- **Assume in constrained type elimination.** Assumes cannot
  appear in pure contexts (contracts, postconditions). Field reads
  appear in pure contexts. Unsuitable.
...
**Decision: Predefined Core Factory read functions with fixed axioms
over an opaque type parameter.**

The Core Factory defines `readInt32`, `readInt8`, etc. as functions
over an opaque type parameter (like Sequence operations over opaque
`Sequence` type). The axioms are fixed — the bounds cannot be
changed by the translator. The translator wires the Box type into
the opaque parameter and calls the appropriate function.

```
function readInt32(box: T) : int    -- T is opaque
  axiom: readInt32(box) >= -2147483648
  axiom: readInt32(box) <= 2147483647
  axiom: readInt32(box) == Box..intVal!(box)
```

The third axiom (tying the result to the Box accessor) may need
to be handled differently since Box is dynamic. Options:
- The function body IS `Box..intVal!(box)` (transparent function)
  and the prover unfolds it
- The equality axiom is generated by the translator (bounded trust)
- The function is purely axiomatic (no body, no equality axiom)
  and the prover uses only the bounds

**Trust boundary:** Only the translator calls these functions. The
translator is the trust boundary. Verifying the translator proves
the functions are called correctly. Until then, the pattern is
auditable: a small set of predefined functions, called in specific
patterns by the translator.

**Implementation note:** The Factory functions need to be
registered with the SMT encoder so it knows how to handle them.
This follows the same pattern as Sequence and Map operations.
The stripping workaround remains in place until this is
implemented.
