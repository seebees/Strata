...
partial def highTypeValToArg : HighType → Arg
  ...
  | .TSequence _et => laurelOp "compositeType" #[ident "Sequence"]
...
where
  stmtExprValToArg : StmtExpr → Arg
    ...
    | .TryCatch body _ catchBody =>
      let catchArg := optionArg (catchBody.map stmtExprToArg)
      laurelOp "block" #[stmtExprToArg body, catchArg]
    | .Throw expr => stmtExprToArg expr
